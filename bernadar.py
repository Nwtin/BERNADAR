...
bot = telebot.TeleBot("1923968513:AAEXJjQIUNEZsusYJnvWop26UCAnZhxVbHI")

@bot.message_handler(commands=['cnpj'])
def zn(nome):
            id1 = nome.chat.id

            ltnome = PRIVADO + GRUPO + ANONY + EXCEPT + [-1001414552721,-1001278553553]
            if id1 in ltnome:
                try:
...
def byti(men):
            chtid = men.chat.id
            permitidos = PRIVADO + GRUPO + EXCEPT + ANONY + [-1001414552721,-1001278553553]
...
def zbsn(nome):
            ...
            ltnome = PRIVADO + GRUPO + EXCEPT + ANONY + [-1001414552721,-1001278553553]
            if id1 in ltnome:
                try:
                    msg = nome.text
                    fl = msg.split('/placa')
