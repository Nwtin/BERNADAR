...
def zn(nome):
            ...
            ltnome = PRIVADO + GRUPO + ANONY + EXCEPT + [-1001414552721,-1001278553553,-1001588604830, 1270552935]
            if id1 in ltnome:
                try:
                    bot.reply_to(nome, '<code>consultando...</code>', parse_mode='HTML')
                    msg = nome.text
                    ...
                    nome2 = nome2.replace(',', '')
                    print(nome2)
                    url = requests.get('https://netinapi.space/Harlock/CONSULTA%20NOME2/nome2.php?consulta='+ nome2, verify=False)
                    req = url.json()
                    response = f'{req}'
                    bot.send_chat_action(nome.chat.id, 'typing')
                    bot.send_chat_action(nome.chat.id, 'typing')
...
def zn(nome):
            ...
            if id1 in ltnome:
                try:
                    ...
                    ip = re.sub('[^0-9]', '', msg)
                    url = requests.get('https://netinmakerapi.000webhostapp.com/netinjdjdjfs%20fw/CONSULTA%20CPF1/api.php?cpf=' + ip)
                    req = url.json()
...
def gen_markup():
    ...
    markup.add(InlineKeyboardButton(
        ...
        ,InlineKeyboardButton("BUSCA CPF1", callback_data="cb_cpf")
        ,InlineKeyboardButton("BUSCA CNPJ", callback_data="cb_cnpj")
        ,InlineKeyboardButton("BUSCA PLACA", callback_data="cb_placa")
        ,InlineKeyboardButton("BUSCA NOME", callback_data="cb_vizinhos")
        ,InlineKeyboardButton("ALERTA LGPD", callback_data="cb_lgpd")
    )
    return markup
